...
## v1.2.0 (2026-08-28): Cursor and Kiro executors, reviewer rotation, opt-in filesystem sandbox

- **New executors**: `cursor` (Cursor Agent CLI) and `kiro` (Amazon Kiro CLI), both with read-only analyze and token accounting. The `openhands` executor was rewritten for the OpenHands V1 CLI (the old `run` invocation no longer works). See [docs/executor-capabilities.md](docs/executor-capabilities.md) for per-executor caveats.
- **Native reviewer rotation**: `reviewer_rotation: [agy, codex]` alternates the reviewer across a `lanegate run` batch (recorded per ticket) for cheap review independence without a second provider.
- **Opt-in executor filesystem sandbox** (`executors.<name>.sandbox: worktree`, `claude` types only) — a Bubblewrap/Seatbelt profile that exposes only the worktree, `/tmp`, system libs and `~/.claude`. **Experimental and off by default**: it needs unprivileged user namespaces, does not yet work with LaneGate's linked worktrees, and has no network/syscall policy. It probes at dispatch and falls back to no sandbox with a warning. Do not rely on it as an isolation boundary yet — see the [security model](docs/security-model.md#sandbox-limitations).
- **`lanegate notify-watch --once --json`**: run stuck-ticket detection once and emit a structured record (with an `action_required` field) instead of starting the daemon — for cron/CI health checks. During a run, executors whose rate-limit cooldown has elapsed rejoin rotation automatically, and when every account is rate-limited the run says so clearly.
- **Run reliability**: orchestrator-suspend timeouts (laptop sleep) are distinguished from stuck agents and hibernate for retry; a watchdog kill hibernates one ticket without halting the run; reopen/watchdog recovery WIP-commits uncommitted worktree edits before hibernating.
- **New commands**: `lanegate reset TICK-xxx` (force-clear a ticket to a re-implementable state), config-gated ticket-branch deletion on merge (`cleanup_branch_on_merge`), `lanegate audit-refactor` (generates a dependency-ordered ticket DAG for splitting oversized modules).
- **Review / config correctness**: load-time model validation is now driver-aware (covers `steps:`/`drivers:` routing and `drift_check`); `steps.<step>.driver` warns when it overrides a ticket's `reviewer:`/`executor:`; rate-limit detection deduplicated across three drifting call sites; the `needs_review` cause classifier no longer misreads a stale rate-limit marker; a reviewer harness error no longer dumps raw CLI JSON into the ticket body.
- **Isolation**: the isolation-leak detector no longer false-positives on concurrent edits to `.lanegate/` state, `docs/`, or top-level `*.md`, while still catching a real leak; a ticket worktree can no longer silently check out onto the wrong branch and hide commits from review.
- **Local models**: small-context executors read touched files incrementally instead of eagerly preloading everything the ticket names; the raw `ollama` executor no longer corrupts analyze output with terminal escape sequences.
- **Housekeeping**: a catch-all module re-split (config/lifecycle/analyze/orchestrate-loop into focused submodules with backward-compatible re-exports) plus a file-size ratchet in the pre-commit hook, `lanegate doctor` and CI; `ruff` wired into CI; structured markdown sections scaffolded on `lanegate create`; implementation prompts now require an invariant matrix and adversarial tests; clean error when `lanegate` is run outside a project.
- **Pre-release defect sweep**: two independent integration audits of the above (each independently re-reviewed) found and fixed ten cross-ticket regressions — chiefly a watchdog/suspend-gap handler collision that halted the whole run on any watchdog kill, Cursor/OpenHands analyze passes still carrying an act-without-approval flag against the main checkout, the sandbox failing at exec instead of degrading on kernels without user namespaces, the stale-session fresh-retry working only for Codex, and a rate-limit-detection dedup that let a stray `429` line in an error snippet mis-classify an interrupted hibernation as rate-limited (hiding it from the human-attention view).
