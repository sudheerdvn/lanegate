...
TICK-385 renamed the project from Vyuha to LaneGate: the Python package is now `lanegate`, the CLI binary is `lanegate` (was `vyuha`), and the config filename/state directory are derived from `APP_NAME = "lanegate"` (see `lanegate/__init__.py` and `lanegate/config.py`). If you initialized a repo with an older `vyuha` install, its on-disk state still uses the old names and needs a one-time manual move — LaneGate does not rewrite another tool's files automatically.
