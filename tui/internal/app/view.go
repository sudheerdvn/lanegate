...
func (m *Model) footerKeys(vp *ui.Viewport) []string {
	...
	case screenRun:
		if m.run.IsHistoryDetail() {
			...
			keys = append(keys, "j/k select", "enter open", "a activity/audit", "c copy all", "v/y copy range")
		}
...
func (m *Model) footerPageInfo() string {
	if m.helpVisible || m.screen != screenRun || !m.run.IsAuditMode() {
		return ""
	}
...
func (m *Model) helpBody() string {
	...
	b.WriteString("  1-6        switch screens\n")
